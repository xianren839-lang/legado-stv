...
object SourceHelp {
     ...
     * 内置书源URL，不可被用户删除
     */
    val BUILT_IN_BOOK_SOURCE_URLS = setOf("https://sangtacviet.me")

    fun isBuiltInBookSource(url: String?): Boolean {
        return url != null && url in BUILT_IN_BOOK_SOURCE_URLS
    }
    ...
    fun deleteBookSourceParts(sources: List<BookSourcePart>) {
        val deletable = sources.filter { !isBuiltInBookSource(it.bookSourceUrl) }
    ...
    fun deleteBookSource(key: String) {
        if (isBuiltInBookSource(key)) return
    ...
    fun adjustSortNumber() {
        if (
            ...
            val builtInSources = sources.filter { isBuiltInBookSource(it.bookSourceUrl) }
            val userSources = sources.filter { !isBuiltInBookSource(it.bookSourceUrl) }
            userSources.forEachIndexed { index, bookSource ->
                bookSource.customOrder = index
            }
            appDb.bookSourceDao.upOrder(userSources)
            builtInSources.forEach { source ->
                appDb.bookSourceDao.upOrder(source.bookSourceUrl, Int.MIN_VALUE / 2)
            }
        }
    }
