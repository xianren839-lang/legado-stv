...
interface JsExtensions : JsEncodeUtils {
    ...
    /**
     * 通过WebView获取STV章节密钥
     * 复刻STV Flutter版 ChapterKeyService 的WebView流程:
     * 1. 加载 app.v2.php 初始化环境
     * 2. 注入Capacitor HTTP模拟层
     * 3. 获取并执行grantcontext JS
     * 4. 调用 app.reader.getKey 获取章节密钥
     */
    @SuppressLint("SetJavaScriptEnabled", "JavascriptInterface")
    fun getChapterKey(host: String, bookId: String): String {
        if (isMainThread) {
            error("getChapterKey must be called on a background thread")
        }
        rhinoContext.ensureActive()
        val source = getSource()
        val baseUrl = source?.getKey() ?: "https://sangtacviet.me"
        val sourceHeaders = source?.getHeaderMap(true) ?: emptyMap()
        ...
        runOnUI {
            ...
            settings.userAgentString = sourceHeaders["User-Agent"]
                ?: AppConst.androidId
            webView.addJavascriptInterface(
                HttpShim(baseUrl, sourceHeaders, webView),
                "FlutterHttp"
            )
            webView.addJavascriptInterface(
                KeyShim(resultDeferred),
                "FlutterKey"
            )
                        ...
                        window.Capacitor.Plugins.Http = {
                            get: function(opts) {
                                return new Promise(function(resolve, reject) {
                                    window.__flutterHttpResolve = function(body) {
                                        resolve({ data: body, headers: {} });
                                    };
                                    window.__flutterHttpReject = function(err) {
                                        reject(new Error(err));
                                    };
                                    FlutterHttp.postMessage(JSON.stringify({ url: opts.url }));
                                });
                            }
                        };
                    """.trimIndent()
                    view.evaluateJavascript(initJs) {
                        val fetchGcJs = """
                            (function() {
                                try {
                                    var xhr = new XMLHttpRequest();
                                    xhr.open('GET', '$baseUrl/io/grantcontext/context?hostid=$host&bookid=$bookId', false);
                                    xhr.send();
                                    if (xhr.status === 200) {
                                ...
                                try {
                                    app.reader.getKey('$host', '$bookId').then(function() {
                                        var key = app.reader.chapterkey || '';
                                        FlutterKey.postMessage(key);
                                    }).catch(function(e) {
                                        FlutterKey.postMessage('');
                                    });
                                } catch(e) {
                                    FlutterKey.postMessage('');
                                }
                        ...
                        """.trimIndent()
                        view.evaluateJavascript(fetchGcJs, null)
                    }
                }
            }
            webView.loadUrl("$baseUrl/app.v2.php", sourceHeaders)
            Thread({
                try {
                    val key = runBlocking {
                        withTimeout(30000L) { resultDeferred.await() }
                    }
                    runOnUI { webView.destroy() }
                } catch (_: Exception) {
                    runOnUI { webView.destroy() }
            ...
            }, "stv-key-timeout").start()
        }
        return runBlocking(context) {
            withTimeout(35000L) { resultDeferred.await() }
        }
    }
    ...
    class HttpShim(
        ...
        fun postMessage(jsonMsg: String) {
            try {
                val msg = GSON.fromJsonObject<Map<String, String>>(jsonMsg).getOrNull()
                val url = msg?.get("url") ?: return
                val absUrl = if (url.startsWith("http")) url else "$baseUrl${if (url.startsWith("/")) "" else "/"}$url"
                val cookie = AndroidCookieManager.getInstance().getCookie(absUrl)
                val reqBuilder = Request.Builder().url(absUrl).get()
                sourceHeaders.forEach { (k, v) -> if (k != "Cookie") reqBuilder.addHeader(k, v) }
                if (!cookie.isNullOrEmpty()) reqBuilder.addHeader("Cookie", cookie)
                val client = OkHttpClient.Builder().followRedirects(true).build()
                val resp = client.newCall(reqBuilder.build()).execute()
                val setCookies = resp.headers("Set-Cookie")
                if (setCookies.isNotEmpty()) {
                    val cm = AndroidCookieManager.getInstance()
                    setCookies.forEach { cm.setCookie(absUrl, it.split(";")[0]) }
                }
                val body = resp.body?.string() ?: ""
                val escaped = body.replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n").replace("\r", "\\r")
                webView.post {
                    webView.evaluateJavascript("__flutterHttpResolve('$escaped')", null)
                }
            } catch (e: Exception) {
                webView.post {
                    webView.evaluateJavascript("__flutterHttpReject('${e.message?.replace("'", "") ?: "error"}')", null)
                }
            }
        }
    }
    ...
    @Keep
    class KeyShim(private val deferred: CompletableDeferred<String>) {
        @JavascriptInterface
        fun postMessage(key: String) {
